...
public class TestNeueKlassenstruktur01{
	...
	public void testeGeschaeftsobjekt(){
		...
		if(Reflections.containsMethodName(Geschaeftsobjekt.class, "setNr"))
			fail("Die Nr eines Geschaeftsobjekts darf nach Objekterzeugung nicht mehr veraendert werden (read-only)!");
		if(Reflections.containsMethodName(Geschaeftsobjekt.class, "setNummer"))
			fail("Die Nr eines Geschaeftsobjekts darf nach Objekterzeugung nicht mehr veraendert werden (read-only)!");
		if(!Reflections.containsMethodName(Geschaeftsobjekt.class, "getNr"))
			fail("Die Klasse Geschaeftsobjekt hat keinen Getter fuer das Attribut \"Nr\" (getNr() fehlt)!");
	...
	public void testeKunde(){
		...
		for(String method : required)
			if(!Reflections.containsMethodName(Kunde.class, method))
		...
		if(! Kunde.class.getSuperclass().getName().equals("geschaeftsobjekt.Geschaeftsobjekt") )
	...
	public void testeProdukt(){
		...
		for(String method : required)
			if(!Reflections.containsMethodName(Produkt.class, method))
				fail("Die Klasse Produkt hat keine Methode \"" + method + "\"!");
		if(! Produkt.class.getSuperclass().getName().equals("geschaeftsobjekt.Geschaeftsobjekt") )
		...
		assertEquals("Fehlerhafte Implementierung von getNr() der Klasse Produkt (nach Erzeugung eines neuen Artikels)", 12345, a.getNr());
	...
	public void testeArtikel(){
		// Vererbungsbeziehung?
		if(! Artikel.class.getSuperclass().getName().equals("geschaeftsobjekt.Produkt") )
	...
	public void testeDienstleistung(){
		// Vererbungsbeziehung?
		if(! Dienstleistung.class.getSuperclass().getName().equals("geschaeftsobjekt.Produkt") )
